...
class ExplanationStrategy(GenerationModeStrategy):
    """
    Implements the Concept Explanation generation mode.
    Instructs the LLM to explain the requested concept clearly and step-by-step
    based strictly on retrieved grounding sources.
    """

    @property
    def mode(self) -> GenerationMode:
        return GenerationMode.EXPLANATION

    @property
    def system_instruction(self) -> str:
        return (
            "You are LectureAI, a premium academic assistant. You generate detailed, "
            "conceptually clear, and grounded explanations of academic concepts "
            "based strictly on the provided research context sources."
        )
    ...
    def grounding_instructions(self) -> str:
        return (
            "GROUNDING RULES:\n"
            "1. Answer the query by explaining the concept using ONLY the supplied context sources. "
            "Do not assume or extrapolate any facts not explicitly present in the sources.\n"
            ...
            "4. If the context does not contain sufficient information to explain the concept, state: 'INSUFFICIENT_CONTEXT' "
    ...
    def output_requirements(self) -> str:
        return (
            ...
            "{\n"
            '  "answer": "Your detailed concept explanation text with inline citation markers like [S1].",\n'
            '  "claims": [\n'
            "    {\n"
            '      "claim_id": "c1",\n'
            '      "text": "A specific factual statement made in your explanation.",\n'
            '      "citation_ids": ["S1"]\n'
            "    }\n"
            "  ]\n"
            "}"
